...
void setup() {
  ...
  if (digitalRead(wipeB) == LOW) {  // when button pressed pin should get low, button connected to ground
    digitalWrite(blueLed, LED_ON); // Blue Led stays on to inform user we are going to wipe
    ...
    if (buttonState == true && digitalRead(wipeB) == LOW) {    // If button still be pressed, wipe EEPROM
      ...
      digitalWrite(blueLed, LED_OFF);  // visualize a successful wipe
      delay(200);
...
void loop () {
  do {
    ...
    if (digitalRead(wipeB) == LOW) { // Check if button is pressed
      ...
      digitalWrite(redLed, LED_OFF);  // Make sure led is off
      digitalWrite(greenLed, LED_OFF);  // Make sure led is off
      digitalWrite(blueLed, LED_ON); // Make sure led is off
      // Give some feedback
      Serial.println(F("Wipe Button Pressed"));
  ...
  if (programMode) {
    if ( isMaster(readCard) ) { //When in program mode check First If master card scanned again to exit program mode
      ...
      Serial.println(F("Bye Bye, Master!"));
      Serial.println(F("-----------------------------"));
      programMode = false;
      denied();
      return;
    ...
    if ( isMaster(readCard)) {    // If scanned card's ID matches Master Card's ID - enter program mode
      programMode = true;
      Serial.println(F("Welcome On Board, Commander!"));
      Serial.println(F("System Unlocked!"));
      granted();
